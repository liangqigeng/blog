<?php 
    ...
    if (!empty($_GET['sin_id'])) {
        ...
        $list = $db->join_all_more('single','find','article','sin_id','art_id','*',"b.sin_id = $sin_id ORDER BY sin_addtime ASC LIMIT 0,6");
        $smarty->assign('list',$list);
    } else {
        $count = $db->select_count_more('article','cat_art','cat_id','a.cat_id = 1');
         //文章列表
        $list = $db->join_all('article','cat_art','cat_id','*','a.cat_id = 1 ORDER BY art_addtime ASC LIMIT 0,6');
        $smarty->assign('list',$list);
    }
    ...
    //指向指定模板显示
	$smarty->display('ganhuo.tpl',$_SERVER['REQUEST_URI']);
?>
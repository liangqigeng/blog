<?php 
	...
	//轮播文章查询
	$art_photo = $db->select_all('article','art_id,art_img','1 ORDER BY art_ord ASC Limit 0,3');
	$smarty->assign('art_photo',$art_photo);
	//原创栏图片
    $orign = $db->select_one('banner','*','banner_id = 4');
    $smarty->assign('orign',$orign);
    //原创文章查询
    $orign_article = $db->join_all_more('article','find','single','art_id','sin_id','a.art_id,c.sin_id,art_title','1 ORDER BY art_addtime ASC LIMIT 0,6');
    $smarty->assign('orign_article',$orign_article);
    //标签栏图片
    ...
    if (!empty($_GET['sin_id'])) {
        $sin_id = $_GET['sin_id'];
        $count = $db->select_count_morer('single','find','article','sin_id','art_id','b.sin_id = $sin_id');
        ...
        $count = $db->select_count('article');
         //文章列表
        $list = $db->select_all('article','*',"1 ORDER BY art_addtime ASC LIMIT $con,$limit");
        $smarty->assign('list',$list);
    }
